...
def print_recommendations(df, day_summary, cat_summary):
    ...
    for _, row in day_summary.iterrows():
        if row['выполнение_плана_%'] < 80:
            print(f"   - {row['день_недели']}: выполнение плана {row['выполнение_плана_%']:.1f}% – ниже целевого. Скорректируйте план вниз.")
            has_day_deviations = True
        elif row['выполнение_плана_%'] > 120:
            print(f"   - {row['день_недели']}: выполнение плана {row['выполнение_плана_%']:.1f}% – превышение. Увеличьте план.")
            has_day_deviations = True
    ...
    print(f"   - Перерасход: {len(df[df['статус'] == 'Перерасход'])} записей – увеличьте план и остатки для этих позиций.")
